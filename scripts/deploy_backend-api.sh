...
printf "Deploying the service: kubectl apply -f ${K8S_DIR}/backend-api-service.yaml \n"
kubectl apply -f ${K8S_DIR}/backend-api-service.yaml

#COMMAND TO DO BOTH AT SAME TIME:
#kubectl apply -f "$K8S_DIR/app-deployment.yaml" -f "$K8S_DIR/backend-api-service.yaml"
